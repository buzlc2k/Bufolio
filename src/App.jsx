...
function App() {
    ...
    // Intro Slide
    const IntroSlide = () => (
        <div className="h-screen flex items-center justify-center px-8 md:px-12 relative overflow-hidden">
            ...
            <div className="max-w-5xl text-center relative z-10">
                ...
                <motion.h1
                    ...
                    className="text-6xl md:text-7xl font-bold mb-6 text-text-primary pixel-text-vn"
                >
                    Phạm Bá Đức
                </motion.h1>
                ...
                <motion.div
                    ...
                    className="h-1 w-32 bg-accent mx-auto mb-12"
                />
                ...
                <motion.p
                    ...
                    className="text-lg md:text-xl text-text-secondary mb-16 leading-relaxed max-w-4xl mx-auto"
                >
                ...
                <motion.div
                    ...
                    className="flex gap-6 justify-center text-text-secondary"
                >
                    <motion.a
                        href="https://github.com/buzlc2k"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-4 rounded-lg border-4 border-border bg-bg-secondary hover:bg-accent hover:text-white hover:border-accent transition-all duration-200 shadow-pixel"
                        whileHover={{ y: -3 }}
                    >
                        <Github size={32} />
                    </motion.a>
    ...
    const ProjectSlide = ({ project }) => (
        <div className="h-screen flex items-center justify-center px-8 md:px-16 py-20 relative overflow-hidden">
            ...
            <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-5 gap-12 items-center relative z-10">
                ...
                >
                    <div className="aspect-video rounded-lg overflow-hidden border-4 border-border bg-gray-100 shadow-card pixel-border">
                        <img
                ...
                >
                    <h2 className="text-3xl md:text-4xl font-bold mb-4 text-text-primary pixel-text">
                        {project.title}
                    </h2>
                    ...
                    <div className="flex flex-wrap gap-3 mb-6">
                        {project.tags.map((tag, i) => {
                            ...
                            return (
                                <span
                                    key={i}
                                    className={`px-4 py-2 text-white font-bold pixel-tag`}
                                    style={{
                    ...
                    {/* Description */}
                    <div className="mb-6">
                        <h3 className="text-sm font-bold text-text-primary mb-2 uppercase tracking-wide">Description</h3>
                    ...
                    <div className="mb-8">
                        <h3 className="text-sm font-bold text-text-primary mb-3 uppercase tracking-wide">Key Technical Features</h3>
                        <ul className="space-y-2 text-xs md:text-sm text-text-secondary max-h-48 overflow-y-auto pr-2">
                            {project.features.map((feature, i) => (
                                <li key={i} className="flex items-start">
                                    <span className="text-accent mr-2 mt-0.5 text-base pixel-bullet flex-shrink-0">▪</span>
                                    <span>{feature}</span>
                                </li>
                            ))}
                        </ul>
                    ...
                    <div className="flex flex-wrap gap-3">
                        {project.links.github && (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="btn text-xs flex items-center gap-2 pixel-btn"
                            >
                                <Github size={14} />
                                GitHub
                            </a>
                        ...
                        {project.links.video && (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="btn-outline text-xs flex items-center gap-2 pixel-btn"
                            >
                                <Play size={14} />
                                Video
                            </a>
    ...
    return (
        <div className="relative overflow-hidden">
            ...
            <button
                onClick={prevSlide}
                className="fixed left-8 top-1/2 -translate-y-1/2 z-50 p-4 bg-bg-secondary border-4 border-border rounded-lg shadow-card hover:shadow-card-hover transition-all hover:bg-accent hover:text-white hover:border-accent pixel-nav"
            ...
            <button
                onClick={nextSlide}
                className="fixed right-8 top-1/2 -translate-y-1/2 z-50 p-4 bg-bg-secondary border-4 border-border rounded-lg shadow-card hover:shadow-card-hover transition-all hover:bg-accent hover:text-white hover:border-accent pixel-nav"
            ...
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 flex gap-3">
                {Array.from({ length: totalSlides }).map((_, index) => (
                    <button
                        ...
                        className={`h-3 rounded-full transition-all border-2 pixel-indicator ${currentSlide === index
                            ? 'bg-accent w-12 border-accent'
                            : 'bg-border w-3 border-border hover:bg-text-secondary hover:border-text-secondary'
                            }`}
